...
class MAPPOUtils:
    ...
    def flatten_joint_obs(obs_tuple):
        return np.concatenate(
            [MAPPOUtils.flatten_agent_obs(o) for o in obs_tuple], axis=0
        ).astype(np.float32)
    ...
    #    Shared team reward with safety-focused shaping.

    #    1) Team reward from mean environment reward
    #    2) Small lane-change incentive
    #    3) Unsafe-distance penalty
    #    4) Safe-spacing bonus
    #    5) Strong crash penalty
    #    6) Cooperation through one shared reward
  
        rewards = (
            list(base_reward)
            if isinstance(base_reward, (tuple, list, np.ndarray))
            else [float(base_reward)]
        )
        team_reward =float(np.mean(rewards))

        # Reward shaping hyperparameters
        lane_change_bonus = 0.00
        unsafe_dist_threshold = 20.0
        unsafe_dist_penalty = 0.4
        spacing_bonus = 0.05
        crash_penalty = 20.0
    ...
    @staticmethod
    def calculate_gae(rewards, values, gamma=0.99, lam=0.95):
        values_ext = np.append(values, 0.0).astype(np.float32)
    ...
    def rollout(model, env, max_steps=500):
        ...
        # Per-timestep buffers for team critic / GAE
        timestep_rewards = []
        timestep_values = []
        ...
        for _ in range(max_steps):
            ...
            # Actor: local observation for each controlled vehicle
            for agent_obs in local_obs:
            ...
            # Centralized critic: one joint value per timestep
            with torch.no_grad():
                value = float(model.value(joint_obs_tensor).item())
            ...
            if Hyperparameters.RENDER and getattr(env, "render_mode", None) == "human":
                env.render()
                time.sleep(1 / 15)

            # Store team-level reward/value ONCE per timestep
            timestep_rewards.append(team_reward)
            timestep_values.append(value)

            # Store per-agent policy samples
            for agent_idx in range(n_agents):
                local_obs_buffer.append(local_obs[agent_idx])
                joint_obs_buffer.append(joint_obs)
                actions_buffer.append(actions[agent_idx])
        ...
        # Repeat team advantages/returns for each agent
        advantages = np.repeat(advantages_t, n_agents).astype(np.float32)
        returns = np.repeat(returns_t, n_agents).astype(np.float32)
